...
def draw_glyph(svg: ET.Element, d_contents: str, gradient_id: str, group_id:str = 'glyph-group'):
    group = ET.SubElement(svg, 'g', {
        'id': group_id,
        'transform': 'scale(.75)'
    })
...
def handle_color_map(c_map: dict, icon_name: str, glyph_d_contents: str | None = None):
    ...
    #local = locals().values()
    #print(local)
    #values = [v for v in local.values()]
    #print(values)
    #return

    # construir + normalizar o caminho final e gerar o svg
    final = OUTPUT_DIR / icon_name
...
handle_color_map(map_blue, 'leros')