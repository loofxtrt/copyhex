...
def draw_glyph(
    ...
    #    attributes['gradient-transform'] = gradient_transform
...
def structure_svg(
    ...
    glyph_fill = handle_fill(
        ...
        gradient_transform=DEFAULT_GLYPH_GRADIENT_TRANSFORM,
    )
    ...
    if glyph is not None:
        ...
        attributes = {}
        # se o transform não for explicitamente none, significa que ele deve ser aplicado
        # isso implica que mesmo que use só o transform normal, ainda tem que hardcodar scale(.75)
        # TODO: fazer a ausência do transform indicar default, mas none explícito indicar nenhum transform
        ...
        for p in glyph.get('paths'):
            d = p.get('d')
            t = p.get('transform')
            #gt = p.get('gradient-transform', DEFAULT_GLYPH_GRADIENT_TRANSFORM)
            draw_glyph(
                group=glyph_group,
                fill=glyph_fill,
                d=d,
                transform=t
                #gradient_transform=gt
            )
